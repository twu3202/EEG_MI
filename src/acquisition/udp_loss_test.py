...
# ----------------------------------------------------------------- live capture
def run_live(host, port, fs, seconds, window):
...
# ----------------------------------------------------------------- demo
def run_demo(fs, seconds, window, loss=0.03, seed=0):
...
def report(acc, dur, fs, out, demo=False):
    ...
    print(f"  丢失 lost          : {acc.lost}   (from seq-counter jumps)")
    print(f"  应收 expected      : ~{expected:.0f}   (recv+lost = {acc.total})")
    print(f"  丢包率 LOSS RATE   : {acc.loss_pct:.3f} %")
    print(f"  有效采样率 eff. Hz : {eff:.1f}  (标称 {fs})")
    verdict = ("✅ excellent (<0.5%)" if acc.loss_pct < 0.5 else
               "🟡 acceptable (0.5–2%)" if acc.loss_pct < 2 else
               "🟠 marginal (2–5%) — 换独立热点/靠近/减少干扰" if acc.loss_pct < 5 else
               "🔴 bad (>5%) — WiFi 链路有问题")
    print(f"  评价 verdict       : {verdict}")
    print("=" * 60)
...
def main():
    ...
    if args.demo:
        ...
        acc, dur = run_demo(args.sfreq, seconds, args.window, loss=args.loss)
    else:
        out = args.out
        acc, dur = run_live(args.host, args.port, args.sfreq, seconds, args.window)
        if acc.recv == 0:
