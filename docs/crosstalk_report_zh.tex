...
\section{稳健性检验:零分布的背景不是均匀的}
\label{sec:nulls}

检出检验的零分布是把同一个估计器放在 \SIrange{4}{10}{\hertz} 上的 75 个频率上拟合得到的。这段
带宽有好几赫兹,而被测通道的噪声在其中\emph{并不平坦}:幅度谱的 log--log 斜率在 $-0.77$ 到
$-1.21$ 之间(中位 $-0.99$),也就是常见的 $1/f$,\SIrange{4}{5}{\hertz} 比
\SIrange{9}{10}{\hertz} 高 \SI{2.2}{} 倍。从一个倾斜背景上取的零分布混入了不属于 $f_0$ 的电平,
原则上会同时影响 $p$ 值和上界。

\begin{figure}[h]\centering
\includegraphics{../results/crosstalk7_nulls.png}
\caption{左:零分布带内的背景。每个悬空通道都服从 $1/f$;唯一那个终接通道在同一段里是白的
(斜率 $+0.21$)—— 一个正常偏置的输入本就该是这样。中:用宽零分布算的上界,与紧贴 $f_0$ 的
局部零分布对比。右:被驱动通道的拟合幅度对「距 $f_0$ 的距离」—— 单调衰减,那是它自身音调的
谱泄漏,在不携带音调的通道上不存在。}
\end{figure}

\paragraph{查了,而且不影响结论。}改用\emph{局部}零分布重跑 —— $f_0 \pm
\SIrange{0.3}{1.0}{\hertz}$,280 个频率,这个带宽窄到 $1/f$ 倾斜可以忽略 —— 每个被测通道的上界
最多移动 \SI{0.32}{\deci\bel},而 $p$ 值\emph{上升}(如 AF3 $0.440 \to 0.500$,
P8 $0.453 \to 0.536$)。也就是说在局部零分布下结论反而略强一点,并不依赖于用哪一段带宽。

\paragraph{唯一真正受影响的地方。}在\emph{被驱动}通道上,两种零分布差了 \SI{4.8}{\deci\bel}
(宽 \SI{0.066}{} 对局部 \SI{0.116}{\micro\volt}),而这个差异是有物理意义的、不是干扰项:它是
那个 \SI{24.5}{\micro\volt} 音调的谱泄漏。拟合幅度随距离单调衰减 —— $|\Delta f| = 0.1$、$0.2$、
$0.45$、$0.8$、$1.5$、\SI{2.5}{\hertz} 处分别是 \SI{0.281}{}、\SI{0.134}{}、\SI{0.065}{}、
\SI{0.039}{}、\SI{0.029}{}、\SI{0.023}{\micro\volt} —— 而悬空通道在同样这些区间里平坦地保持在
$\approx$\,\SI{1.1}{\micro\volt}。

由此得到的是一条规则,不是一处修正:\textbf{要外推终接被测通道的噪声本底,零分布必须取在远离
音调的地方,绝不能取在紧挨着音调的地方。}终接的被测通道自己不带音调,也就没有东西可泄漏;正确
的代理量是被驱动通道\emph{远离} $f_0$ 处的本底,即 \SIrange{0.023}{0.066}{\micro\volt}。
第~\ref{sec:protocol} 节里 \SI{-51}{\deci\bel} 的外推用的是这个区间保守的一端,依然成立。
